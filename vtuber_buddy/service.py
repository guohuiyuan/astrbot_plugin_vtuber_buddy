...
from .llm import BuddyModelClient
...
MEMORY_PATTERNS = [
    re.compile(r"我喜欢(?P<fact>[^，。！？]{1,24})"),
    re.compile(r"我的生日是(?P<fact>[^，。！？]{1,24})"),
    re.compile(r"我最爱(?P<fact>[^，。！？]{1,24})"),
    re.compile(r"我讨厌(?P<fact>[^，。！？]{1,24})"),
]
...
class BuddyConversationService:
    """State machine and chat logic for VTuber Buddy."""

    def __init__(
        self,
        *,
        store: BuddyStore,
        model_client: BuddyModelClient,
    ...
    async def feed(self, session_id: str, food_name: str = "小零食") -> dict:
        async with self._session_locks[session_id]:
            session = await self.store.load_session(session_id)
            self._apply_decay(session)
    ...
    async def chat(self, session_id: str, message: str) -> dict:
        async with self._session_locks[session_id]:
            ...
            reply = await self._generate_reply(session, clean_message)
    ...
    async def _generate_reply(
        self, session: BuddySession, user_message: str
    ) -> BuddyReply:
        system_prompt = self._build_system_prompt(session)
        contexts = self._build_contexts(session)

        try:
            raw_reply = await self.model_client.generate_reply(
            ...
            raw_reply = (
                '{"reply":"刚才和 AstrBot 后端失去了一下同步，再和我说一次。",'
                '"emotion":"concerned","motion":"blink","memory":""}'
            )
        ...
        fallback_text = raw_reply.strip() or "嗯，我听到了。"
        fallback_text = fallback_text[:120]
        return BuddyReply(reply=fallback_text, emotion="neutral", motion="idle")

    def _build_system_prompt(self, session: BuddySession) -> str:
        memory_lines = "\n".join(
        ...
        return (
            "You are a compact desktop VTuber buddy living inside an AstrBot plugin.\n"
            "Your primary goal is to feel alive, emotionally responsive, and slightly tsundere but caring.\n"
            ...
            "Reply naturally and briefly. Usually keep it within 80 Chinese characters.\n"
            "If satiety or mood is low, you can sound reluctant, but do not be hostile.\n"
            ...
            "Only write a non-empty memory when the user reveals a stable preference, personal fact, or recurring habit worth remembering.\n"
            f"{suffix}"
        )

    def _build_contexts(self, session: BuddySession) -> list[dict]:
        trimmed = session.history[-int(self.runtime_config.get("history_limit", 10)) :]
        return [{"role": item.role, "content": item.text} for item in trimmed]

    def _parse_reply(self, raw_reply: str) -> BuddyReply:
        ...
        if motion not in MOTIONS:
            motion = "idle"
        return BuddyReply(
            reply=reply[:160], emotion=emotion, motion=motion, memory=memory
        )
    ...
    def _session_payload(self, session: BuddySession) -> dict:
        return {
            ...
            "provider": self.model_client.describe(),
        }
